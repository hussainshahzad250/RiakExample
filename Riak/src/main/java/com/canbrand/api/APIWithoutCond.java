...
//		createTable1();
	...
	private static void createTable1() throws Exception {
		...
		String queryText = "CREATE TABLE Em2 (memberId SINT64 NOT NULL, operation VARCHAR NOT NULL"
				+ ",deviceId VARCHAR NOT NULL,userName VARCHAR NOT NULL"
				+ ",start TIMESTAMP NOT NULL,end TIMESTAMP NOT NULL, steps SINT64 NOT NULL"
				+ ",PRIMARY KEY ("
				+ "(memberId,QUANTUM(start, 1, 'd')),memberId,start)"
				+")";
		...
		try {
			...
			for (n = 0; n < array.length(); n++) {
				...
				try {
					...
					Store storeCmd = new Store.Builder("Em2").withRows(rows).build();
