...
public class CreateAndInsert {
	...
	private static void createTable1() throws Exception {
		...
		String queryText = "CREATE TABLE Emp5 (memberId SINT64 NOT NULL, operation VARCHAR NOT NULL, deviceId VARCHAR NOT NULL,userName VARCHAR NOT NULL, PRIMARY KEY (memberId))";
	...
	private static void insertData()
		...
		try {
			...
			List<Row> rows = Arrays
					.asList(new Row(new Cell(obj.getInt("memberId")), new Cell(obj.getString("operation")),
							new Cell(obj.getString("deviceId")), new Cell(obj.getString("userName"))));

			Store storeCmd = new Store.Builder("Emp5").withRows(rows).build();
	...
	private static void insertDataToTable2() throws Exception {
		...
		try {
			...
			for (int n = 0; n < array.length(); n++) {
				...
				List<Row> rows = Arrays
						.asList(new Row(new Cell(object.getInt("memberId")), Cell.newTimestamp(startDate),
								Cell.newTimestamp(endDate), new Cell(object.getInt("steps"))));
